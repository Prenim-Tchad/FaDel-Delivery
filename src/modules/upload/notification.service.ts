...
export class NotificationService {
   ...
   */
  async sendOrderConfirmation(to: string, orderId: string, amount: number, neighborhood: string) {
   ...
   */
  async sendDriverAlert(to: string, distance: string, price: number, pickup: string) {
  ...
  private async sendMessage(to: string, body: string, useWhatsApp: boolean) {
    ...
    try {
      const result = await this.client.messages.create({ body, from, to: recipient });
