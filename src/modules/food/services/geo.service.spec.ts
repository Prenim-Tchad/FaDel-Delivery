...
describe('GeoService', () => {
  ...
  describe('calculateHaversineDistance', () => {
    it('doit calculer correctement la distance entre deux points connus', () => {
      ...
      const lat1 = 12.091; const lon1 = 15.053;
      const lat2 = 12.110; const lon2 = 15.045;
  ...
  describe('getRoadDistanceAndDuration (Google Maps)', () => {
    it('doit retourner les données de l API Google', async () => {
      ...
      const destination = { lat: 12.110, lng: 15.045 };

      try {
        const result = await service.getRoadDistanceAndDuration(origin, destination);
        
        expect(result).toHaveProperty('distanceKm');
        expect(result).toHaveProperty('durationMinutes');
        console.log(`Résultat Google : ${result.textDistance} en ${result.textDuration}`);
      } catch (error) {
