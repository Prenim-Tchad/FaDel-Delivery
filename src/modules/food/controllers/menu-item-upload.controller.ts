 ...
 *
 * Route :
 * POST /food/menu-items/:id/upload-photo → upload photo + thumbnail
 */
...
export class MenuItemUploadController {
   ...
   * Upload une photo pour un article de menu
   * Génère automatiquement un thumbnail 150×150 px
   */
  @Post('menu-items/:id/upload-photo')
  @UseInterceptors(
    FileInterceptor('photo', {
      // Stockage en mémoire (buffer) — on upload directement sur R2
      storage: undefined,
      limits: {
  ...
  @ApiConsumes('multipart/form-data') // indique que la route accepte des fichiers
  @ApiOperation({
    summary: 'Uploader une photo pour un article de menu',
    description:
      'Upload une image (JPEG/PNG/WebP, max 5MB). Génère automatiquement un thumbnail 150×150 px. Retourne les URLs de l\'original et du thumbnail.',
  })
  @ApiParam({
    name: 'id',
