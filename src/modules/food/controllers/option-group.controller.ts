...
export class OptionGroupController {
  ...
  @ApiOperation({
    ...
    description: "Crée une option (ex: 'Grande', 500 FCFA) dans un groupe existant.",
  })
  @ApiParam({ name: 'id', description: "ID du groupe d'options", example: 'clxxx123' })
  @ApiResponse({ status: HttpStatus.CREATED, description: 'Option créée avec succès' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: "Groupe d'options introuvable" })
  ...
  @ApiParam({ name: 'id', description: "ID du groupe d'options", example: 'clxxx123' })
  @ApiResponse({ status: HttpStatus.OK, description: "Liste des options du groupe" })
