...
export class FcmService implements OnModuleInit {
  ...
  onModuleInit() {
    try {
      ...
      if (fs.existsSync(servicePath)) {
        ...
        this.logger.warn(
          'Fichier firebase-adminsdk.json manquant, FCM désactivé',
        );
      }
    } catch (error: unknown) {
  ...
  async sendMulticast(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'FCM Multicast Error';
      throw new Error(errorMessage);
    }
  }
  ...
  async sendToTopic(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'FCM Topic Error';
      throw new Error(errorMessage);
    }
  }
}
